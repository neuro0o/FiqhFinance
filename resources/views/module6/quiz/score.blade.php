...
<!-- TITLE -->
@section('title', 'Module 6 Score')

@section('page-css')
    <link rel="stylesheet" href="{{ asset('css/utils/sidebar.css') }}">
    <link rel="stylesheet" href="{{ asset('css/moduleMiniGame/moduleMiniGame6.css') }}">
@endsection

@section('content')
    @include('layouts.partials.sidebar')
    <div class="score-content" style="padding:2rem;">
        <h1>Module 6: Result</h1>

        @if(!$result || !isset($result['score']))
            <div class="alert alert-info">No result available. Try the quiz first.</div>
            <a href="{{ route('module6.quiz.start') }}" class="btn btn-primary">Start Quiz</a>
        @else
            <div style="margin-top:1rem;">
                <h2><span>You Scored: {{ $result['score'] }} / {{ $result['total'] }}</span></h2>

                @if($result['isNewBest'])
                    <p><strong>🎉 New Best Score!</strong><br>Your best for Module 6 is now <strong>{{ $result['bestScore'] }} / {{ $result['total']}}.</strong></p>
                @else
                ...
                @else
                    <p class="no-badge">You didn't earn a badge this time.<br>Try to get at least <strong>7/10</strong> to earn <strong>Shariah Planner</strong>!</p>
                @endif


                <div class="score-button" style="margin-top:1.25rem;">
                    <a href="{{ route('module6.quiz.start') }}" class="btn btn-primary">Play Again</a>
                    <a href="{{ route('module6.note') }}" class="btn btn-secondary">Back to Notes</a>
                </div>
            </div>
        @endif
    </div><br><br><br><br>
